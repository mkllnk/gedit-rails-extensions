...
check_file() {
  ...
  *.xml)
    xmllint --relaxng /usr/share/gtksourceview-2.0/language-specs/styles.rng --noout $file || exit 1
    ;;
  *)
    xmllint --relaxng /usr/share/gtksourceview-2.0/language-specs/language2.rng --noout $file || exit 1
    ;;
  esac
}
